...
CRS_M <- config$project$crs  # Denmark ETRS89 / UTM zone 32N

# Denmark bounding box (WGS84)
bbox_config <- config$project$denmark_bbox
dk_bbox_wgs84 <- st_bbox(c(
  xmin = bbox_config$xmin,
  ymin = bbox_config$ymin,
  xmax = bbox_config$xmax,
  ymax = bbox_config$ymax
), crs = bbox_config$crs)
...
cat("Analysis period:", config$analysis$time_period, "\n")
```
...
# Load CSV data
road_kill_dk <- read_csv(here(config$data$raw$roadkill$path)) %>%
...
roads_cache <- here(config$data$processed$roads_cached$path)
...
if (file.exists(roads_cache)) {
  ...
  # Load shapefile
  roads_raw <- st_read(here(config$data$raw$roads$path), quiet = TRUE) %>%
    st_transform(CRS_M) %>%
    st_crop(dk_bbox_proj)
...
traffic_cache <- here(config$data$processed$traffic_cached$path)
...
if (file.exists(traffic_cache)) {
  ...
  # Load shapefile
  traffic_raw <- st_read(here(config$data$raw$traffic$path), quiet = TRUE) %>%
    st_transform(CRS_M) %>%
    st_crop(dk_bbox_proj)
...
landuse_cache <- here(config$data$processed$landuse_cached$path)
...
if (file.exists(landuse_cache)) {
  ...
  # Load shapefile
  landuse_raw <- st_read(here(config$data$raw$landuse$path), quiet = TRUE) %>%
    st_transform(CRS_M) %>%
    st_crop(dk_bbox_proj)
...
car_codes <- c(
  config$data$raw$roads$car_accessible_codes$major,
  config$data$raw$roads$car_accessible_codes$minor,
  config$data$raw$roads$car_accessible_codes$links
)
...
distances_cache_file <- here(config$data$processed$distances_cache$path)
...
threshold_percentile <- config$analysis$traffic_matching$distance_threshold_percentile
...
corine_cache_file <- here(config$data$processed$corine_cached$path)
...
if (file.exists(corine_cache_file)) {
  ...
  corine_raw <- rast(here(config$data$raw$corine$path))
...
landcover_cache_file <- here(config$data$processed$landcover_props$path)
...
if (file.exists(landcover_cache_file)) {
  ...
  buffer_dist <- config$analysis$road_buffer_distance
...
roadkill_cache_file <- here(config$data$processed$roadkill_cache$path)
...
model_data <- roads_traf %>%
  ...
  mutate(
    ...
    road_type = case_when(
      code %in% config$data$raw$roads$car_accessible_codes$major ~ "Major",
      code %in% config$data$raw$roads$car_accessible_codes$minor ~ "Minor",
      code %in% config$data$raw$roads$car_accessible_codes$links ~ "Links/Ramps",
      TRUE ~ "Other"
    ),
...
for (pred in config$analysis$model$predictors) {
  cat("  -", pred, "\n")
}
```
...
cat("Distribution:", config$analysis$model$distribution, "\n")
cat("Zero component:", config$analysis$model$zero_dist, "\n")
cat("Offset:", config$analysis$model$offset, "\n\n")
...
hurdle_model <- hurdle(
  ...
  dist = config$analysis$model$distribution,
  zero.dist = config$analysis$model$zero_dist
)
